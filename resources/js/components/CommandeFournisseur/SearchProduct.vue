<template>
    <div class="dp noselect position-relative" style="z-index: 2">
        ...
        <transition name="trans-search">
            ...
            <div class="row search" v-if="showSearch">
                ...
                <div class="input_search">
                    ...
                    <section class="nodata p-2" v-if="products.length == 0">
                        <p>nous n'avons trouvé aucun client.</p>
                    </section>
                </div>

                <ul class="" v-if="products.length > 0" >
                    <li v-for ="product in products " :key="product">
                        <div class="container d-block" @click="selectProduct(product)">
                            <div class="d-flex">
                                <div class="col-6 body_medium text-capitalize text-nowrap">
                                    {{ product.name }}
                                </div>
                                <div class="col-6">
                                    <b class="email body_regular">{{ product.email ? product.email.toLowerCase() : ''}}</b>
                                </div>
                            </div>
                            <div class="d-flex mt-2">
                                <div class="col-4 pe-2 eclipe-item">{{ product.contactname }}</div>
                                <div class="col-4 pe-2 eclipe-item">{{ product.phone }}</div>
                                <div class="col-4 eclipe-item">&euro;{{ product.total }}</div>
                            </div>
                        </div>
                    </li>
                </ul>
                ...
                <div class="col">
                    <span v-if="countProducts > 0" class="show-more body_medium" @click="loadMore()">
                        {{ countProducts }} more products 
                    </span>
                </div>
...
<script setup>
    ...
    function submit(e) {
        ...
        timeout.value = setTimeout(function() {
            ...
            nextTick(() => {
                store.dispatch(`${COMMANDE_FOURNISSEUR_LIST_MODULE}${FOURNISSEUR_SEARCH_PRODUCTS}`, { showmore: 1, search: e.target.value })
                .then((response)=>{
                    if(e.target.value) {
    ...
    function loadMore() {
        store.dispatch(`${COMMANDE_FOURNISSEUR_LIST_MODULE}${FOURNISSEUR_SEARCH_PRODUCTS}`, { showmore: 1, search: search.value })
        .finally(()=>{})
    }

    const selectProduct = (customer) => {
        emit("selected", customer)
        showSearch.value = false
    }

    const products = computed(()=>{
        return store.getters[`${COMMANDE_FOURNISSEUR_LIST_MODULE}products`]
    })

    const countProducts = computed(()=>{
        return store.getters[`${COMMANDE_FOURNISSEUR_LIST_MODULE}productCount`]
    })
