...
const order_states = computed(() => store.getters[`${FOURNISSEUR_STATUS_MODULE}orderStates`])
const loaded = computed(() => store.getters[`${FOURNISSEUR_STATUS_MODULE}loaded`])
...
onMounted(async ()=> {
    if(loaded.value === false) {
        await store.dispatch(`${FOURNISSEUR_STATUS_MODULE}${ORDERSTATETAG_LOAD_ORDER_STATES}`) 
    }
